...
const sendMessage = async (messageObj, uuid) => {
    try {
        await apiClient('http://localhost:8080/api/v1/messages', {
            method: 'POST',
            body: JSON.stringify({uuid, ...messageObj}),
        });
        console.log('Сообщение отправлено:', {uuid, ...messageObj});
        return null;
    } catch (err) {
...
const ChatPage = () => {
    const [useAttachment, setUseAttachment] = React.useState(false);
    ...
    useEffect(() => {
        if (uuid) {
            const fetchMessages = async (retryCount = 3) => {
                ...
                try {
                    const data = await apiClient(`http://localhost:8080/api/v1/messages/text?uuid=${uuid}&page=${page}&size=20`);
    ...
    useEffect(() => {
        if (uuid) {
            ...
            const client = new Client({
                ...
                onConnect: () => {
                    console.log(`WebSocket подключён для чата и получения фото ${uuid}`);
                    client.subscribe(`/topic/chat/${uuid}/llmPhotoResponses`, (msg) => {
                        let body;
                        try {
                            if (msg.binaryBody) {
    ...
    useEffect(() => {
        if (uuid) {
            ...
            const client = new Client({
                ...
                connectHeaders: {Authorization: `Bearer ${token}`},
                onConnect: () => {
                    console.log(`WebSocket подключён для чата и получения текстовых сообщений ${uuid}`);
                        ...
                        try {
                            ...
                            setMessagesByChat((prev) => ({
                                ...
                                [uuid]: [
                                    ...
                                    {
                                        type: 'bot',
                                        content: 'Ошибка обработки ответа сервера',
    ...
    const images = [
        'https://cdn.wallpapersafari.com/56/12/08RXrh.jpg',
        'https://cdn.wallpapersafari.com/56/12/08RXrh.jpg',
        'https://cdn.wallpapersafari.com/56/12/08RXrh.jpg',
        'https://cdn.wallpapersafari.com/56/12/08RXrh.jpg',
        'https://cdn.wallpapersafari.com/56/12/08RXrh.jpg',
    ...
    const handleSend = async () => {
        ...
        setError(null);
        try {
            await sendMessage({content: message, needImages: useAttachment}, uuid);
        } catch (err) {
            setMessagesByChat((prev) => ({
                ...prev,
                [uuid]: [
                ...
                <div className={`chat-sidebar ${sidebarVisible ? 'visible' : 'hidden'}`}>
                    <div className="chat-header">
                        ...
                        <div className="chat-header-buttons">
                            <button className="send-button" onClick={openModal}>Вложения</button>
                    ...
                    <div className="chat-input">
                        <div className="input-with-button">
                            <input
                                ...
                                placeholder="Что если..."
                                onKeyDown={(e) => e.key === 'Enter' && handleSend()}
                                disabled={loading || !uuid}
                            />

                            <button
                                className={`inline-button ${useAttachment ? 'active' : ''}`}
                                onClick={() => setUseAttachment((prev) => !prev)}
                                type="button"
                            ...
                            >
                                ...
                                >
                                    <path
                                        ...
                                        strokeLinejoin="round"
                                        strokeWidth={2}
                                        d="M16.5 7.5l-7 7a3 3 0 01-4.24-4.24l7-7a2 2 0 112.83 2.83L7.5 12.5"
                                    />
                                </svg>
                                <span className="inline-label">Сгенерировать</span>
                            </button>
                        </div>
